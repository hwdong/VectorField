...
public:
	...
	int type; T d;
public:
	VFDesign_Element(const VFDesign_Element<T > &ele) {
		p = ele.p; d = ele.d; type = ele.type;
	}
...
class VFDesign_SingularElement :VFDesign_Element<T>
{
	T k;
public:
	VFDesign_SingularElement(const VFDesign_SingularElement<T> &ele):VFDesign_Element<T>(ele) {	
		k = ele.k;
	}
...
public:
	VFDesign_ResularElement(const VFDesign_ResularElement<T > &ele) :VFDesign_Element<T>(ele) {			
		v = ele.v;
	}
...
template<typename T>
inline bool readDesignElements(std::vector<VFDesign_Element<T>*> &eles, const char *filename)
{
...
#include <fstream>
template<typename T>
inline void test_gen_vector_field_ByElements(VectorField<T> *&vField
