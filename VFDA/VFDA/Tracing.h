...
template<typename T>
void Vertex2NextTriangle(const VectorField<T> &VF, const int vert_id, int &face_id,
	const Eigen::Matrix<T, 1, Dynamic> &pV)
{
	...
	const bool forwardbackward)
{
	...
	pV.normalize();
	Vertex2NextTriangle(VF,vert_id,face_id,pV);
}
...
inline void test_GetBarycentricFacters() {
...
//-------------------------trace tragitory-------------------
template<typename T>
inline void generate_circle_field(T *dx, T *dy, int width, int height)
{
	T cx = width*0.5;
	T cy = height*0.5;;
...
#include <fstream>
inline void test_TraceInATriangle() {
